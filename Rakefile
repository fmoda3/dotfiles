require 'rake'
require 'fileutils'
require File.join(File.dirname(__FILE__), 'bin', 'moda', 'vundle')

desc "Hook our dotfiles into system-standard positions."
task :install => [:submodule_init, :submodules] do
  puts
  puts "======================================================"
  puts "Welcome to MODA Installation."
  puts "======================================================"
  puts
  ...
  file_operation(Dir.glob('zsh/config/*')) if want_to_install?('zsh configs')
...
task :submodules do
  unless ENV["SKIP_SUBMODULES"]
    puts "======================================================"
    puts "Downloading MODA submodules...please wait"
    puts "======================================================"
...
def install_tmuxinator
  run %{which tmuxinator}
  puts "======================================================"
   puts "Installing Tmuxinator...If it's already installed,    "
...
def install_fonts
  ...
  puts "======================================================"
  run %{ cp -f $HOME/.moda/fonts/* $HOME/Library/Fonts }
  puts
end
...
def install_prezto
  ...
  unless File.exists?(File.join(ENV['ZDOTDIR'] || ENV['HOME'], ".zprezto"))
    run %{ ln -nfs "$HOME/.moda/zsh/prezto" "${ZDOTDIR:-$HOME}/.zprezto" }
  ...
  puts
  puts "Overriding prezto ~/.zpreztorc with MODA's zpreztorc to enable additional modules..."
  run %{ ln -nfs "$HOME/.moda/zsh/prezto-override/zpreztorc" "${ZDOTDIR:-$HOME}/.zpreztorc" }
...
def file_operation(files, method = :symlink)
  files.each do |f|
    ...
    # This modifies zshrc to load all of moda's zsh extensions.
    # Eventually moda's zsh extensions should be ported to prezto modules.
    if file == 'zshrc'
      File.open(target, 'a') do |zshrc|
        zshrc.puts('for config_file ($HOME/.moda/zsh/*.zsh) source $config_file')
      end
    end
...
def success_msg(action)
  puts ""
  puts "   __  __  ____  _____           "
  puts "  |  \/  |/ __ \|  __ \   /\     "
  puts "  | \  / | |  | | |  | | /  \    "
  puts "  | |\/| | |  | | |  | |/ /\ \   "
  puts "  | |  | | |__| | |__| / ____ \  "
  puts "  |_|  |_|\____/|_____/_/    \_\ "
  puts ""
  puts "MODA has been #{action}. Please restart your terminal and vim."
end
